...
@pytest.mark.parametrize("key,attr,inverted", [
    ("local_context", "send_full_history", True),
    ("system_prompt", "send_system_prompt", False),
    ("tool_definitions", "send_tool_definitions", False),
])
def test_context_payload_roundtrip_stays_in_owning_profile(tmp_path, monkeypatch, key, attr, inverted):
    from hermes_cli.web_server_config import CONFIG_SCHEMA

    schema = CONFIG_SCHEMA[f"context.{attr}"]
    assert schema["type"] == "boolean"
    assert bool(schema.get("inverted")) is inverted
    launch, worker = _homes(tmp_path)
    _bind_homes(monkeypatch, launch, worker)
    launch_agent = SimpleNamespace(**{attr: True})
    worker_agent = SimpleNamespace(**{attr: True})
    monkeypatch.setattr(server, "_sessions", {
        "launch": {"agent": launch_agent},
        "worker": {"agent": worker_agent, "profile_home": str(worker)},
    })
    params = {"key": key, "profile": "code"}
    default_word = "off" if inverted else "on"
    assert _get(params)["result"]["value"] == default_word
    assert _set({**params, "value": "toggle"})["result"]["value"] != default_word
    assert _read_yaml(worker)["context"][attr] is False
    assert "context" not in _read_yaml(launch)
    assert getattr(worker_agent, attr) is False
    assert getattr(launch_agent, attr) is True
    assert _get(params)["result"]["value"] == _set({**params, "value": "status"})["result"]["value"]
    assert _set({**params, "value": default_word})["result"]["value"] == default_word
    assert getattr(worker_agent, attr) is True
    assert _read_yaml(worker)["context"][attr] is True
