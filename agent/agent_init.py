...
def _apply_agent_section(agent, _agent_cfg):
    # Payload preferences belong to each agent, since profiles share a process.
    context_cfg = _cfg_dict(_agent_cfg, "context")
    for key in ("send_full_history", "send_system_prompt", "send_tool_definitions"):
        setattr(agent, key, is_truthy_value(context_cfg.get(key, True)))
