...
def _midturn_request_pressure_tokens(
    ...
    needed (#96995).
    """
    tools = agent.tools if getattr(agent, "send_tool_definitions", True) is not False else []
    try:
    ...
    return approx_tokens + (_estimate_tools_tokens_rough(tools) if tools else 0)
