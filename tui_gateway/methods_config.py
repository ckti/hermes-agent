...
def _context_payload_enabled(key):
    ...
    enabled = is_truthy_value(context.get(config_key, True))
    return not enabled if inverted else enabled


def _cfg_get_context_payload(params):
    return {"value": "on" if _context_payload_enabled(params["key"]) else "off"}


# key -> getter(params); bind_module rebinds the table's functions onto server.py's globals.
_CONFIG_GETTERS = {
    **dict.fromkeys(_CONTEXT_PAYLOAD_SETTINGS, _cfg_get_context_payload),
