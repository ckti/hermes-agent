...
def _set_context_payload(rid, params, key, value, session):
    from hermes_constants import hermes_home_key

    current = _context_payload_enabled(key)
    raw = _raw_word(value)
    if raw == "status":
        return _kv(rid, key, "on" if current else "off")
    enabled = not current if raw in {"", "toggle"} else _BOOL_WORDS.get(raw)
    if enabled is None:
        return _err(rid, 4002, f"unknown {key} mode (use on|off|status)")

    config_key, inverted = _CONTEXT_PAYLOAD_SETTINGS[key]
    persisted = not enabled if inverted else enabled
    _write_config_key(f"context.{config_key}", persisted)
    # Multiplexed gateways own agents from several profiles. Apply the switch
    # only to agents belonging to the profile whose config was just written.
    target_home = hermes_home_key()
    for live_session in list(_sessions.values()):
        if hermes_home_key(live_session.get("profile_home") or _hermes_home) != target_home:
            continue
        live_agent = live_session.get("agent")
        if live_agent is not None:
            setattr(live_agent, config_key, persisted)
...
_CONFIG_SETTERS = {
    "local_context": _set_context_payload, "system_prompt": _set_context_payload,
    "tool_definitions": _set_context_payload,
